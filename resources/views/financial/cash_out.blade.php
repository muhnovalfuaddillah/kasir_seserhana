...
<div class="space-y-6 max-w-[1700px] mx-auto p-4 sm:p-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 glass-card p-5 rounded-3xl border border-slate-800 shadow-xl">
        <div class="space-y-1">
            <h1 class="text-xl sm:text-2xl font-black text-white flex items-center gap-2">
                <span class="material-symbols-outlined text-rose-400 text-3xl">north_east</span>
                Pencatatan Kas Keluar & Pengeluaran Operasional
            </h1>
            <p class="text-xs sm:text-sm text-slate-400">Catat beban biaya operasional toko (listrik, air, sewa tempat, gaji, perlengkapan, dan pengeluaran lain).</p>
        </div>
    </div>
    ...
    <div class="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
        ...
        <a href="{{ route('financial.cash_out') }}" class="px-4 py-2.5 rounded-2xl text-xs sm:text-sm font-black whitespace-nowrap bg-rose-600 text-white shadow-glow transition-all flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="glass-card rounded-3xl p-5 sm:p-6 border border-slate-800 space-y-4 shadow-xl">
            <h3 class="text-sm font-black text-white flex items-center gap-2 pb-3 border-b border-slate-800">
                ...
                Form Tambah Kas Keluar / Beban
            </h3>

            <form action="{{ route('financial.cash_out.store') }}" method="POST" class="space-y-4">
                @csrf
                
                <div>
                    <label class="block text-xs font-extrabold text-slate-300 mb-1">Nominal Pengeluaran (Rp) <span class="text-rose-400">*</span></label>
                    <input type="text" name="amount" required data-type="currency" placeholder="Contoh: 250.000" class="input-currency w-full bg-slate-950 text-sm font-black text-rose-400 rounded-xl px-3.5 py-2.5 border border-slate-800 focus:outline-none focus:border-rose-500">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">Kategori Pengeluaran <span class="text-rose-400">*</span></label>
                    <select name="cash_category_id" required data-placeholder="-- Pilih Kategori Beban --" class="select-searchable w-full bg-slate-950 text-xs text-white rounded-xl px-3 py-2 border border-slate-800">
                        <option value=""></option>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-300 mb-1">Sumber Kas</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">No. Bukti / Nota Pembayaran</label>
                    <input type="text" name="reference_number" placeholder="Contoh: NOTA-LISTRIK-0826" class="w-full bg-slate-950 text-xs text-white rounded-xl px-3.5 py-2 border border-slate-800">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">Catatan Keterangan</label>
                    <textarea name="notes" rows="3" placeholder="Keterangan rincian pengeluaran..." class="w-full bg-slate-950 text-xs text-white rounded-xl px-3.5 py-2 border border-slate-800"></textarea>
                </div>
        ...
        <div class="lg:col-span-2 glass-card rounded-3xl p-5 sm:p-6 border border-slate-800 space-y-4 shadow-xl">
            <h3 class="text-sm font-black text-white flex items-center gap-2 pb-3 border-b border-slate-800">
                ...
                Riwayat Transaksi Kas Keluar & Operasional
            </h3>
                    ...
                    <tbody class="divide-y divide-slate-800/80 font-medium">
                        @forelse($transactions as $tx)
                            <tr class="hover:bg-slate-800/40 transition-colors">
                                ...
                                <td class="p-3.5 text-white font-bold">{{ $tx->category->name ?? 'Operasional' }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="p-8 text-center text-slate-500">Belum ada transaksi kas keluar yang dicatat.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
